...
if os.path.exists(history_file):
    ...
    df_history = pd.DataFrame(columns=["Date", "Time", "Amount (LAK)", "Reference", "Ticket", "Receiver", "Text"])
...
def extract_info(image):
    ...
    reference_match = re.search(r"(\d{15,20})", text)
    ticket_match = re.search(r"[A-Z0-9]{10,}", text)
...
if results:
    ...
    st.subheader("รายการสลิปที่อัปโหลด")
    st.dataframe(df)
